...
def initial(root, fx_ref=None, fy_ref=None):
    ...
    filtered = AngularBatch(lam=532e-9,
                            ...
                            fx_ref=fx_ref,
                            fy_ref=fy_ref,
                            # cut_size=[2025, 2148, 466, 180],
                            cut_size=[2108, 2163, 300, 150],
...
class focus_and_z():
    ...
    def func(self, z, **kwargs):
        ...
        score = focus_value(self.model, propped_3d, compress=1)  # 聚焦分数初始化，在重建时已经compress了
...
# @timer
def focus_value(model, image, compress=8):
    model.eval()

    # GPU版本处理
    # 下采样（替代 OpenCV 的 resize）
    h, w = image.shape[0], image.shape[1]
    new_h, new_w = h // compress, w // compress  # todo 测试固定分辨率
    image = F.interpolate(
    ...
    # 边缘检测图像保存
    # result.save(r'F:\dongjiayao\Data\AutoFocusDatabase\edge\edge_{:.3f}.jpg'.format(time.time()))
...
def main(root, save_dir, lb, ub, model=None, fx_ref=None, fy_ref=None):
    dlep_model = model  # 使用外部传入的模型
    filtered = initial(root=root, fx_ref=fx_ref, fy_ref=fy_ref)
    focus_z = focus_and_z(filtered, dlep_model)  # todo get_score_directly表示是否用模型直接得到聚焦分数
    ...
    finder = PeakFinder(max_func, lb=lb, ub=ub, max_evals=200, target='min', precision_factor=1,
...
def batch_main(root_dir, save_dir, test_csv=None):
    ...
    checkpoint = torch.load(r'F:\dongjiayao\Pycharm\DEP-AF\edge_prediction\tmp\rcf\speckle_aug_true\checkpoint_epoch10.pth',
                            weights_only=True, )
    ...
    total_abs_error = 0.0
    total_rel_error = 0.0
    valid_rel_count = 0  # 有效相对误差计数（避免除以零的情况）
    processed_count = 0  # 已处理的有真实值的图像数量
    ...
    if results:
        ...
        # 计算统计指标
        abs_errors = [r['abs_error'] for r in results if r['abs_error'] != '']
        rel_errors = [r['rel_error'] for r in results if r['rel_error'] != '' and not math.isinf(r['rel_error'])]
        ...
        if abs_errors:
            ...
            avg_abs_error = min_abs_error = max_abs_error = 0
...
if __name__ == "__main__":
    test_focusing_accuracy(root_dir=r'F:\dongjiayao\Data\AutoFocusDatabase\hologram_amplitude\hologram',
                           save_dir=r'F:\dongjiayao\Data\AutoFocusDatabase\article\comparison\ours',
                           test_csv=r'F:\dongjiayao\Data\AutoFocusDatabase\AutoFocusDatabase_amplitude.csv')
