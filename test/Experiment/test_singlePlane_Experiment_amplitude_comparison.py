...
class focus_and_z():
    ...
    def func(self, z, **kwargs):
        ...
        # score = -focus_improved_dif_single(propped)
...
def main(root, save_dir, lb, ub, model=None, fx_ref=None, fy_ref=None):
    model = model  # 使用外部传入的模型
    filtered = initial(root=root, fx_ref=fx_ref, fy_ref=fy_ref)
    focus_z = focus_and_z(filtered, model)
    ...
    finder = PeakFinder(max_func, lb=lb, ub=ub, max_evals=50, target='min', precision_factor=1,
...
if __name__ == "__main__":
    test_focusing_accuracy(root_dir=r'F:\dongjiayao\Data\AutoFocusDatabase\hologram_amplitude\hologram',
                           save_dir=r'F:\dongjiayao\Data\AutoFocusDatabase\article\comparison\eigen',
                           test_csv=r'F:\dongjiayao\Data\AutoFocusDatabase\AutoFocusDatabase.csv')
