...
class DexiNed(nn.Module):
    ...
    def forward(self, x):
        ...
        results = [F.interpolate(r, size=(H, W), mode='bilinear', align_corners=False) for r in results]
        ...
        border_mask = create_border_mask(H, W, border_size=50, device=x.device)

        results = [torch.sigmoid(out) * border_mask for out in results]
        return results
