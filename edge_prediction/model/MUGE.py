...
class MUGE(nn.Module):
    def __init__(self, args=None, encoder_name="efficientnet-b7", encoder_weights="imagenet", in_channels=3, classes=1):
        super(MUGE, self).__init__()
    ...
    def forward(self, x):
        ...
        # --- 修改处：上采样到原图尺寸 ---
        if results.shape[2] != img_H or results.shape[3] != img_W:
            results = F.interpolate(results, size=(img_H, img_W), mode='bilinear', align_corners=True)

        # 然后再进行中心裁剪（此时 results 尺寸已经足够大）
        ...
        results = torch.sigmoid(results) * border_mask
        return [results]  # 输出列表，与 RCF 格式对齐
